...
**Intégrité des donnes enregistrées** (`integrity.py`, migration v10) : `env.step()` **ne valide pas la légalité** — contrat d'un moteur RL — donc une carte absente de la main s'ajoute au pli sans être retirée nulle part, et la donne se rejoue jusqu'au bout avec un décompte faux. C'était le cas : le gestionnaire `play` du solo passait `data["action"]` tel quel (le salon validait déjà). Corrigé le 2026-08-01 des deux côtés du même prédicat.
