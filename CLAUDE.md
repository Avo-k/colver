...
- **DD Solver** (`search/solver.rs`): Alpha-beta with TT, PVS, killer/history heuristics. ~77ms/solve from full deal (4 suits ≈ 310ms), ~13.5ms mid-game. Pool generation: ~244 deals/s on 32 cores with LTO+native (`gen_pool` binary). 1M pool ≈ 68min. Without LTO: ~100 deals/s.
  - **BREAKING (2026-07-23): `quick_tricks` removed — it returned wrong DD values.** It credited a whole run of plain-suit master cards as guaranteed points after checking only once that opponents could not ruff, ignoring that they become void on the next round. That bogus lower bound raised `alpha` above the true value and cut valid lines. Measured on real deals: **25% of `solve_for_trump` values were wrong** (66% of deals had ≥1 wrong suit; median error 6 pts, max 52), and 23–29% of play positions had ≥1 wrong card score. Removing it also made the solver ~10% *faster*, so there is no tradeoff. Any pre-2026-07-23 DD data is affected — notably `data/deals/base_5M.bin` and every score layer derived from it, plus arena/training results that depend on them.
  - Invariant: a sound alpha-beta returns the same value whatever the move order. `test_root_scores_match_independent_solve` enforces it by cross-checking each root score against an independent solve — that is what caught the bug. Keep it green when adding pruning.
...
- **Playgen world sampler** (`playgen/`): causal transformer that autoregressively continues a game from the observer-visible prefix; rolling out to the end reveals hidden hands = a determinized world from the learned posterior, consumed by IS-DD (`playgen_frac`/`playgen_temp`) and the web (Croyances marginals, Annonces conditioned worlds). **v1** (COLVPG01, `models/playgen/playgen_final.bin`, 3.2M params d=256 L=4): trump-canonical suits (perm[trump]=0), play-phase targets only. **v2** (COLVPG02, 10.7M params d=384 L=6): physical suits + full 24-perm augmentation, auction actions as prediction targets through a 43-way bid head (public legal mask) — enables **mid-auction deal sampling** (`generate_deals_from_auction`: bid head completes the auction, playout reveals hands) and bid-policy queries. Training: `train_playgen [--v2]` (offline supervised on COLVGM01 replays), export via `export_playgen [--v2]`; inference auto-detects the format. Verdicts: fixed dets playgen ≫ belief > uniform; at equal time budget belief ≥ playgen (quality×quantity). IS-DD ensemble pool: `belief_frac` (uniform coverage floor) + `cred_alpha` (per-world auction-credibility importance weights judged by the bid net). See [docs/play/playgen.md](docs/play/playgen.md).
- **World-credibility benchmark** (`bench_world_cred` binary): self-supervised sampler eval — would the reference policy (bid NN / DouDou50) replay each observed hidden action with the sampled world's hand? Seeded positions, both phases: `cargo run -p colver-core --bin bench_world_cred --release -- --bid-positions 30 --play-positions 30 --worlds 12 --seed 42`. Reference @playgen-v2-60K: auctions playgen 60%/92% (argmax/top3) vs belief 34%/64% vs uniform 12%/32%; play 85/78/70% argmax (hard constraints already carry most play-phase signal — auctions are where samplers differ).
...
- **NN inference kernels** (`nn_kernels.rs`): shared `dot` / `linear` / `layer_norm` for the pure-Rust nets (`dmc_net`, `bid_net`, `belief_net`). Sums are split across 8 independent accumulator lanes, plus an AVX2-dispatched build — a single-accumulator dot product serialises one ~4-cycle FP add per element and can't be auto-vectorised (Rust never grants float reassociation). **Measured 2026-07-23: DmcNet 926→178µs (5.2×), BidNet 219→33µs (6.6×), BeliefNet 158→23µs (6.3×); DouDou50 self-play 35→176 deals/s.** Numerics move only in the last ulp (max abs deviation 6e-4, no argmax change outside an exact tie). Any new inference net should use these rather than an inline loop. `playgen/infer.rs` has its own equivalent `dot8`.
