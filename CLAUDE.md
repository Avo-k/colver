...
- **DD Solver** (`search/solver.rs`): Alpha-beta with TT, PVS, killer/history heuristics. Measured 2026-08-02 on a fixed corpus, 1 thread: **32,3 ms** pour une donne complète via `solve_with_scores`, 169 µs en mi-partie, **1,4 µs** en finale, ~45 M nœuds/s soit ~22 ns/nœud (minimum sur 5 tours ; **la dispersion par tour est de ~9 %**, donc deux chiffres significatifs et jamais de comparaison entre deux jours). Les chiffres antérieurs (77 / 310 / 13,5 ms) sont périmés et ne nommaient ni corpus ni forme. See [docs/play/dd_solver.md](docs/play/dd_solver.md).
...
- `agent_review.py` → `agent_review` table: what **DouDou50 / Oracle / Dédé (IS-DD)** would have played at every non-forced card, whoever actually played it. ~7-10s per deal at the `COLVER_REVIEW_ISDD_MS` (default 500) per-card IS-DD budget; one game at a time (module semaphore) so replay loads don't pile searches onto the playgen sidecar. **IS-DD is seat-bound** — four instances are built, one per seat, all shown every action, and the one whose seat is to play is the one asked. Asking a single instance would hand it information that seat never had.
  - `agent_review.stream()` is an async generator yielding `("start", total)`, `("move", entry)` per card **in play order**, `("done", blob)`. Each step runs in `asyncio.to_thread`. `Agent.decide` and `Env.solve_scores` release the GIL (the latter only since 2026-08-02) ; **`Env.action_oracle_dd` does not**, so the loop is blocked for one full DD search per card. The measured <45 ms round-trip holds because review positions are mid/endgame (169 µs / 1,4 µs) — la même requête à l'entame serait un blocage de ~32 ms. Abandoning the generator unwinds the lock and semaphore and caches nothing partial.
