...
**Cargo features:** `rand` (default), `parallel` (rayon), `dmc_train` (candle GPU training for DMC + bid NN + belief net), `gpu_server` (the playgen sidecar binary), `solver_stats` (DD node counting — off by default, required by any solver benchmark), `solver_ablation` (runtime switches turning off PVS / killers / history via `COLVER_DD_NO_*` — off by default, measurement only)
...
- **DD Solver** (`search/solver.rs`): Alpha-beta with TT, PVS, killer/history heuristics. Compter **~32 ms** pour une donne complète via `solve_with_scores`, et **quatre ordres de grandeur de moins** en fin de donne. **Le tableau des temps par forme vit à un seul endroit**, [docs/play/dd_solver.md](docs/play/dd_solver.md#performance) — avec le corpus, la méthode et la dispersion de mesure (~9 %) qui dit combien de chiffres ont un sens. Ne pas le recopier ici : les chiffres antérieurs (77 / 310 / 13,5 ms) étaient périmés précisément parce qu'ils vivaient en quatre exemplaires sans corpus déclaré.
  ...
  - **Devant une famille d'idées qui ne diffèrent que par la qualité d'une estimation, mesurer d'abord l'estimation parfaite.** `bench_dd oracle` l'a fait pour l'amorçage de fenêtre en une commande, là où en construire un bon aurait coûté des jours : plafond à 2× avec un seed **exact**, 9,5 % à ±20, 1,6 % à ±40 — famille close. Le motif se rejoue tel quel pour l'ordonnancement et pour les bornes.
