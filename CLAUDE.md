...
- **Reprendre une partie** (solo, **connecté seulement** — `matches.user_id` est la clé) : `matches` était **écrit sans jamais être relu**, et l'état de partie solo ne vivait que dans les locales de `_websocket_session`. Trois entrées vers la reprise, une seule mécanique : `play_status {resume}` à l'arrivée sur Jouer. Le serveur tranche dans cet ordre — (1) une partie **encore vivante sur cette socket** est renvoyée telle quelle (`_play_state_msg`, aller-retour SPA vers l'analyse : rien n'est abandonné) ; (2) sinon `_resume_match` la relit de la base ; (3) dans tous les cas `play_open` liste ce qui reste à reprendre. Liens d'entrée : bandeau en tête du panneau de configuration (`#play-resume`), carte « Parties en cours » de `/compte`, et le lien retour de Rejouer (`replay_loaded.resume`, calculé côté serveur pour ne l'offrir qu'au propriétaire). `?resume=<id>` est **retiré de l'URL dès qu'il est lu** : reprendre coûte une donne, un F5 ne doit pas le rejouer.
  - **Une donne en cours se reprend à son coup près** (2026-08-01) — y compris hors partie, qui est le cas par défaut (`target = 0` ne crée pas de ligne `matches`). La reprise se lit sur `games` : `hands` + `dealer` écrits à la création, `actions` à chaque coup, d'où `db.pending_deal` → `PlaySession(hands=…)` + `replay(actions)` → `server._resume_deal`. Le préfixe repasse par `_record_action`, le même chemin qu'en jeu, donc par `AgentTable.observe` : **les bots n'ont pas d'état à restaurer, ils en ont un qui se déduit du préfixe visible** — DouDou50 est sans état et Dédé rebâtit ses mondes à chaque coup. (Ce que la version précédente de ce paragraphe niait, et qui fondait tout le contraire.) Vérifié bit à bit — position, enchères, plis, mains d'origine — à toutes les coupures, DouDou50 comme Dédé.
    - **Tout ou nulle part** : une action illégale au rejeu veut dire que le journal ne colle pas à la donne ; un préfixe à moitié rejoué serait pire que rien, puisque les coups suivants s'écriraient en base derrière un état qui n'est pas le leur. La ligne est alors effacée (`db.drop_deal`) et la partie repart sur une donne neuve — l'ancien comportement.
    - Ce que ça ferme : **recharger la page pour effacer une donne mal engagée**. La ligne restait `is_complete = 0`, donc invisible du score de partie (`Match.record` jamais atteint) comme de l'Elo (`rate_game` exige `is_complete = 1`) : redonne gratuite, répétable. Il reste ouvert **pour un joueur anonyme** — sans compte, aucune identité à laquelle rattacher la donne (`pending_deal` exige `user_id`) ; l'Elo, lui, ne note que des joueurs identifiés.
    - Renoncer à une donne reste possible et reste gratuit — explicitement (« Abandonner », `drop_deal`) ou en démarrant une donne neuve, qui efface celle en plan pour qu'elle ne soit pas reproposée sans fin. **C'est le garde-fou qui manque encore** (cf. `docs/web_todo.md` §2.7) : à trancher entre marquer la donne chutée pour l'abandonneur et n'en compter que les points cartes déjà pris.
  ...
  - **Les quatre mains ne partent qu'à l'état terminal.** `initial_hands` n'est lu par le client que dans `showEndOfGameReview`, mais le solo l'envoyait dès la distribution — le jeu des trois autres était dans la console pendant toute la donne. C'est `_enrich_terminal_msg` qui les joint, au moment où elles s'affichent ; le salon appliquait déjà la règle (`rooms.py`, `if state["is_terminal"]`). Même famille que la fuite de `/api/games/{id}` fermée la veille : une donne en cours ne divulgue rien.
