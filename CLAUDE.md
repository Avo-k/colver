...
- **La direction est l'information** : en « deux camps », un pli qui descend est à Nord-Sud, un qui monte à Est-Ouest, et rien ne l'écrit. Le vol précède le repeint (sinon on animerait la carte suivante) et se termine sur un `setTimeout`, jamais un `transitionend` — `prefers-reduced-motion` neutralise les transitions, l'événement ne partirait jamais.
