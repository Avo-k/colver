...
**Performance** (2026-08-02, fixed 2 120-position corpus, 1 thread, i9-13900K): full deal via `solve_with_scores` **32.3 ms** · mid-game (13-24 cards left) **169 µs** · endgame (2-12 cards) **1.4 µs** · one IS-DD determinized world **1.13 ms**; ~45 M nodes/s (~22 ns/node), min-of-5 with a ~9 % per-round spread. Value-only `solve_for_trump` on a full deal: ~18 ms mean, 10.4 ms median, 69 ms P95. Full table and method: [play/dd_solver.md](play/dd_solver.md#performance). Invariant: `ns + ew == 162` (or 252 for capot).
